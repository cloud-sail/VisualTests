...
// calc_pixel_material_inputs_analytic_derivatives_other_inputs
	...
	FloatDeriv2 Local1 = ConstructFloatDeriv2(Parameters.TexCoords[0].xy,Parameters.TexCoords_DDX[0].xy,Parameters.TexCoords_DDY[0].xy);
	MaterialFloat Local2 = MaterialStoreTexCoordScale(Parameters, DERIV_BASE_VALUE(Local1), 0);
	MaterialFloat4 Local3 = ProcessMaterialColorTextureLookup(Texture2DSampleGrad(Material.Texture2D_0,Material.Texture2D_0Sampler,DERIV_BASE_VALUE(Local1),(Local1.Ddx)*exp2(View.MaterialTextureMipBias),(Local1.Ddy)*exp2(View.MaterialTextureMipBias)));
	MaterialFloat Local4 = MaterialStoreTexSample(Parameters, Local3, 0);
	FloatDeriv2 Local5 = ConstructFloatDeriv2(Parameters.TexCoords[1].xy,Parameters.TexCoords_DDX[1].xy,Parameters.TexCoords_DDY[1].xy);
	FloatDeriv2 Local6 = ConstructFloatDeriv2(Parameters.TexCoords[2].xy,Parameters.TexCoords_DDX[2].xy,Parameters.TexCoords_DDY[2].xy);
	...
	PixelMaterialInputs.Metallic = (Local5).Value;
	PixelMaterialInputs.Specular = (Local6).Value;
...
// calc_pixel_material_inputs_other_inputs
	...
	MaterialFloat Local2 = MaterialStoreTexCoordScale(Parameters, DERIV_BASE_VALUE(Local1), 0);
	MaterialFloat4 Local3 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material.Texture2D_0,Material.Texture2D_0Sampler,DERIV_BASE_VALUE(Local1),View.MaterialTextureMipBias));
	MaterialFloat Local4 = MaterialStoreTexSample(Parameters, Local3, 0);
	MaterialFloat2 Local5 = Parameters.TexCoords[1].xy;
	MaterialFloat2 Local6 = Parameters.TexCoords[2].xy;
	...
	PixelMaterialInputs.Specular = Local6;
	PixelMaterialInputs.Roughness = 0.50000000f;
	PixelMaterialInputs.Anisotropy = 0.00000000f;
	PixelMaterialInputs.Normal = MaterialFloat3(0.00000000f,0.00000000f,1.00000000f);
	PixelMaterialInputs.Tangent = MaterialFloat3(1.00000000f,0.00000000f,0.00000000f);
...
// get_material_customized_u_vs
	MaterialFloat2 Local7 = Parameters.TexCoords[0].xy;
	MaterialFloat2 Local8 = Parameters.TexCoords[1].xy;
	MaterialFloat2 Local9 = Parameters.TexCoords[2].xy;
	OutTexCoords[0] = Local7;
	OutTexCoords[1] = Local8;
	OutTexCoords[2] = Local9;
...
FMaterialAttributes FMaterialAttributes_SetCustomData0(FMaterialAttributes InAttributes, float InValue) { InAttributes.CustomData0 = InValue; return InAttributes; }
FMaterialAttributes FMaterialAttributes_SetCustomData1(FMaterialAttributes InAttributes, float InValue) { InAttributes.CustomData1 = InValue; return InAttributes; }
...
FMaterialAttributes FMaterialAttributes_SetFirstPersonInterpolationAlpha(FMaterialAttributes InAttributes, float InValue) { InAttributes.FirstPersonInterpolationAlpha = InValue; return InAttributes; }
FMaterialAttributes FMaterialAttributes_SetMaterialCache(FMaterialAttributes InAttributes, float4 InValue) { InAttributes.MaterialCache = InValue; return InAttributes; }
FMaterialAttributes FMaterialAttributes_SetTemporalResponsiveness(FMaterialAttributes InAttributes, float InValue) { InAttributes.TemporalResponsiveness = InValue; return InAttributes; }
FMaterialAttributes FMaterialAttributes_SetMotionVectorWorldOffset(FMaterialAttributes InAttributes, float3 InValue) { InAttributes.MotionVectorWorldOffset = InValue; return InAttributes; }

// material_cache_common

// material_cache_pass

// material_cache_types
...
// num_custom_vertex_interpolators
0
// num_material_texcoords
3
// num_material_texcoords_vertex
3
...
// pixel_material_inputs
	...
	MaterialFloat CustomData0;
	MaterialFloat CustomData1;
...
// Vertex interpolators offsets definition
	...
	#include "/Engine/Public/IndirectVirtualTextureCommon.ush"
#endif
...
// Material shader uses Front material or root node is converted to Substrate data.
#define TEMPLATE_USES_SUBSTRATE					(SUBSTRATE_ENABLED && MATERIAL_IS_SUBSTRATE)
// Material shader is NOT exported to Substrate data (no FrontMaterial or SubstrateTree data). It uses legacy GBuffer data.
#define TEMPLATE_USES_LEGACYGBUFFERDATA			(SUBSTRATE_ENABLED && !MATERIAL_IS_SUBSTRATE)
...
#ifndef ALLOW_SUBSTRATE_PUBLIC_ACCESS
#define ALLOW_SUBSTRATE_PUBLIC_ACCESS 0
#endif

#if !SUBSTRATE_OPAQUE_DEFERRED || ALLOW_SUBSTRATE_PUBLIC_ACCESS
...
{
#if MATERIAL_SHADINGMODEL_SINGLELAYERWATER && !HIT_PROXY_SHADER && !SCENE_TEXTURES_DISABLED && (SINGLE_LAYER_WATER_SHADING_QUALITY != SINGLE_LAYER_WATER_SHADING_QUALITY_MOBILE_WITH_DEPTH_BUFFER)

#if MOBILE_SUPPORTS_SM5_MATERIAL_NODES
	const float4 SLWBufferBilinearUVMinMax = View.BufferSizeAndInvSize.zwzw * (ResolvedView.ViewRectMinAndSize.xyxy + float4(0.55f, 0.55f, ResolvedView.ViewRectMinAndSize.zw - 0.55f));
	const float2 ClampedViewportUV = clamp(ViewportUV, SLWBufferBilinearUVMinMax.xy, SLWBufferBilinearUVMinMax.zw);
	float WaterDeviceZ = WaterSampleSceneDepthWithoutWater(MobileBasePass.SceneDepthCopyTexture, MobileBasePass.SceneDepthCopySampler, ClampedViewportUV, View.BufferSizeAndInvSize.xy, View.BufferSizeAndInvSize.zw);
#else
...
MaterialFloat4 MobileSceneTextureLookup(inout FMaterialPixelParameters Parameters, int SceneTextureId, float2 UV)
{
	...
	case PPI_StoredBaseColor:
		return MaterialFloat4(GBuffer.StoredBaseColor, 0);
	case PPI_StoredSpecular:
		return MaterialFloat4(GBuffer.StoredSpecular.rrr, 0);
...
float4 SceneTextureLookup(float2 UV, int SceneTextureIndex, bool bFiltered)
{
...
#elif MATERIAL_SHADINGMODEL_SINGLELAYERWATER && !HIT_PROXY_SHADER
	...
	#elif SUBSTRATE_ENABLED && (SUBSTRATE_GBUFFER_FORMAT==0 || SUBSTRATE_GBUFFER_FORMAT==1) && !SHADING_PATH_MOBILE // Not on mobile that is not using dbuffer decal but the legacy GBuffer blending.
	{
		#if !SUBSTRATE_OPAQUE_DEFERRED || ALLOW_SUBSTRATE_PUBLIC_ACCESS // Prevents opaque material to sample the Gbuffer.
		FSubstrateGBuffer SubstrateGBuffer = SubstratePublic_GetSubstrateGBufferFromFirstBSDF(PixelPos, false/*bGetNormalizedNormal*/);
		switch (SceneTextureIndex)
		{
			// order needs to match to ESceneTextureId
...
float4 SceneTextureLookupCommon(FMaterialPixelParameters Parameters, float2 UV, int SceneTextureIndex, bool bFiltered)
{
#if SHADING_PATH_MOBILE
	return MobileSceneTextureLookup(Parameters, SceneTextureIndex, UV);
#else
	return SceneTextureLookup(UV, SceneTextureIndex, bFiltered);
#endif
}
...
{
#if SHADING_PATH_MOBILE
	float4 Lookup = MobileSceneTextureLookup(Parameters, SceneTextureIndex, UV);
#else
	float4 Lookup = SceneTextureLookup(UV, SceneTextureIndex, bFiltered);
#endif
#if POST_PROCESS_MATERIAL && POST_PROCESS_MATERIAL_BEFORE_TONEMAP && !POST_PROCESS_DISABLE_PRE_EXPOSURE_SCALE
	if (SceneTextureIndex >= PPI_PostProcessInput0 && SceneTextureIndex <= PPI_PostProcessInput6)
	{
		Lookup.xyz *= View.OneOverPreExposure;
	}
#endif
	return Lookup;
}
...
#if SHADING_PATH_DEFERRED || MOBILE_DEFERRED_SHADING
...
#endif // SHADING_PATH_DEFERRED || MOBILE_DEFERRED_SHADING
...
bool ShouldEnableWorldPositionOffset(FMaterialVertexParameters Parameters)
{
	...
	#if (FEATURE_LEVEL > FEATURE_LEVEL_ES3_1 || MOBILE_SUPPORTS_SM5_MATERIAL_NODES) // Non optional WPO on mobile not supporting sm5 material nodes
...
{
	#if HAVE_GetTemporalResponsivenessOutput0 > 0
		return saturate(GetTemporalResponsivenessOutput0(Parameters));
	#else
		return 0;
	#endif
}

float3 GetMaterialMotionVectorWorldOffset(FMaterialPixelParameters Parameters)
{
	#if HAVE_GetMotionVectorWorldOffsetOutput0 > 0
		return GetMotionVectorWorldOffsetOutput0(Parameters);
	#else
		return float3(0, 0, 0);
	#endif
}
...
float GetUnlitMaterialRefractionIOR(in FMaterialRefractionData RefractionData)
{
...
{
	// Always initialise to avoid later operations to potentially cause compiler failure due to read uninitialized value when get_material_customized_u_vs is not defined.
	UNROLL
	for (int CoordinateIndex = 0; CoordinateIndex < NUM_TEX_COORD_INTERPOLATORS; CoordinateIndex++)
	{
		OutTexCoords[CoordinateIndex] = 0.0f;
	}
